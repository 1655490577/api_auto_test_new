    ...
    try:
        ...
        for i in rsp_bm.json()['data']['list']:
            if i['label'] == f'测试部门{i+1}':
                branch_id = i['id']
                getter.project_add(cookies=cookies, name=f'测试项目{i+1}', mobileType=2, token=token, userid=userId, orgId=branch_id)
        yield
        for i in rsp_bm.json()['data']['list']:
            ...
            for j in rsp_project.json()['data']['list']:
                getter.project_batchDelete(cookies=cookies, id=j['id'], mobileType=2, token=token, userid=userId)
    except Exception as e:
...
def add_station():
    ...
    """
    try:
        rsp_data = getter.get_allPerms(cookies=cookies, pageNum=1, mobileType=2, pageSize=100, token=token, userid=userId)
        for i in range(3):
            rsp_add = getter.station_save(cookies=cookies, name=f'测试岗位{i+1}', perms=getter.get_dict_allIds(rsp_data.json()),
                                          token=token, mobileType=2, userid=userId)
        ...
        getter.station_delete(cookies=cookies, id=station_id, mobileType=2, token=token, userid=userId)
    except Exception as e:
        logger.error(f"添加项目前置执行失败，错误为{e}")
    ...
    try:
        ...
        for i in range(3):
            getter.user_save(cookies=cookies, divisionId=branch_id, jobId=station_id[0], name=f'测试添加账号{i+1}',
                             password='123', phone='13100000002', status=1, mobileType=2, token=token, userid=userId,
